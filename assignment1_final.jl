...
println("The code below has assumed no temperature constraints to exist. This implies that even though the low temperature propertyx of N2 is 300, the code will run even on temperatures below the low temperature. Some lines of code in the get_coefficients function has been commented out which shows how the program would have proceeded if we would have applied the temperature constraints. ")
...
function extract_meta_data(line)
    ...
    species_properties[species] = Species(species, values[3], values[2], values[1], Dict(), Dict(), 0.0, 0.0, 0.0)
...
function second_line(line)
    ...
    while i < 76
        try
            species_properties[species].high_temp_properties["a"*string(floor(Int, i/15 +1))] = parse(Float64,line[i:i+14])
        catch
            return ""
        end
        i+=15
...
function third_line(line)
    ...
    while i < 106
        try
            species_properties[species].high_temp_properties["a"*string(floor(Int, i/15 +1))] = parse(Float64,line[i-75:i-75+14])
        catch
            return ""
        end
        i+=15
    ...
    while i < 46
        try
            species_properties[species].low_temp_properties["a"*string(floor(Int, i/15 +1))] = parse(Float64,line[i+30:i+44])
        catch
            return ""
        end
        i+=15
...
function fourth_line(line)
    ...
    while i < 106
        try
            species_properties[species].low_temp_properties["a"*string(floor(Int, i/15 +1))] = parse(Float64,line[i-45:i-45+14])
        catch
            return ""
        end
        i+=15
...
function get_coefficients(T::Float64, species::String)
    try
        ...
        if T > species_properties[species].common_temp
            coeff = species_properties[species].high_temp_properties
            temp_range = "high_temp_properties"
        else
            coeff  = species_properties[species].low_temp_properties
            temp_range = "low_temp_properties"
        end
        return coeff
    catch
