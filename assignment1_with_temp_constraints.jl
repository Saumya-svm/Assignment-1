...
function get_coefficients(T::Float64, species::String, temp_invalid=false)
    try
        if T > species_properties[species]["common_temp"] && T <= species_properties[species]["high_temp"]
            coeff = species_coefficients[species]["high_temp"]
            temp_range = "high_temp_properties"
        end
        if T <= species_properties[species]["common_temp"] && T >= species_properties[species]["low_temp"]
            coeff  = species_coefficients[species]["low_temp"]
            temp_range = "low_temp_properties"
        end
        return coeff, temp_range, T
...
function calculate_specific_heat(T::Float64, species=species) 
    temp = T
    coeff, temp_range, temp = get_coefficients(temp, species)
    species_specific_heat = R*(coeff["a1"] + coeff["a2"]*temp + coeff["a3"]* (temp^2) + coeff["a4"]*(temp^3) + coeff["a5"]*(temp^4))
    species_properties[species][temp_range]["specific_heat"] = species_specific_heat
    return species_specific_heat, temp_range, temp
end
...
function calculate_enthalpy(T::Float64, species=species)
    temp = T
    coeff, temp_range, temp = get_coefficients(T, species)
    species_enthalpy = R*temp*(coeff["a1"] + coeff["a2"]*(temp)/2 + coeff["a3"]*(temp^2)/3 + coeff["a4"]*(temp^3)/4 + coeff["a5"]*(temp^4)/5 + coeff["a6"]/temp)
    species_properties[species][temp_range]["enthalpy"] = species_enthalpy
    return species_enthalpy, temp_range, temp
end
...
function calculate_entropy(T::Float64, species=species)
    temp = T
    coeff, temp_range = get_coefficients(T, species)
    species_entropy = R*(coeff["a1"]*log(temp) + coeff["a2"]*(temp) + coeff["a3"]* (temp^2)/2 + coeff["a4"]*(temp^3)/3 + coeff["a5"]*(temp^4)/4 + coeff["a7"])
    species_properties[species][temp_range]["entropy"] = species_entropy
    return species_entropy, temp_range, temp
end
...
function calculate_species_properties(T::Float64)
    for i in keys(species_properties)
        global species
        species = i
        species_properties[species]["specific_heat"] = calculate_specific_heat(T)[1]    
        species_properties[species]["enthalpy"] = calculate_enthalpy(T)[1]    
        species_properties[species]["entropy"] = calculate_entropy(T)[1]    
    end
end
...
function calculate_individual_species_property(Species::String, T::Float64)
    ...
    species = Species
    temp = T
    species_properties[species]["specific_heat"] = calculate_specific_heat(temp)[1]  
    species_properties[species]["enthalpy"] = calculate_enthalpy(temp)[1]
    species_properties[species]["entropy"] = calculate_entropy(temp)[1]
...
function calculate_mixture_specific_heat(mixture_comp, T)
    ...
    for i in keys(mixture_comp)
        ...
        if mixture_comp[species] > 0.0
            species_specific_heat, temp_range, new_temp = calculate_specific_heat(temp)
            species_properties[species]["specific_heat"] = species_specific_heat
            if new_temp != temp
                return calculate_mixture_specific_heat(mixture_comp, new_temp)
            end
            mixture_specific_heat += species_specific_heat*mixture_comp[species]
        end
    end
    return mixture_specific_heat, temp
end
...
function calculate_mixture_enthalpy(mixture_comp, T)
    ...
    for i in keys(mixture_comp)
        species = i
        if mixture_comp[species] > 0.0
            species_enthalpy, temp_range, new_temp = calculate_enthalpy(temp)
            species_properties[species]["enthalpy"] = species_enthalpy
            if new_temp != temp
                return calculate_mixture_enthalpy(mixture_comp, new_temp)
            end
            mixture_enthalpy += species_enthalpy*mixture_comp[species]
        end
    end
    return mixture_enthalpy, temp
end
...
function calculate_mixture_entropy(mixture_comp, T)
    ...
    for i in keys(mixture_comp)
        species = i
        if mixture_comp[species] > 0.0
            species_entropy, temp_range, new_temp = calculate_entropy(temp)
            species_properties[species]["entropy"] = species_entropy
            if new_temp != temp
                return calculate_mixture_entropy(mixture_comp, new_temp)
            end
            mixture_entropy += species_entropy*mixture_comp[species]
        end
    end
    return mixture_entropy, temp
end
...
function CalculateMixtureProperties(mixture_comp::Dict, T::Float64)
    temp = T
    mixture_specificheat, temp = calculate_mixture_specific_heat(mixture_comp, temp)
    mixture_enthalpy, temp = calculate_mixture_enthalpy(mixture_comp, temp)
    mixture_enntropy, temp = calculate_mixture_entropy(mixture_comp, temp)

    p = 1000
    println("\nThermodynamic properties of pure species at $temp(K) and $p(Pa)\n")
    println("Species \t Enthalpy(J/mol) \t Entropy(J/mol-K) \t cp(J/mol-K)\n")
    # l = ["CH4", "CO", "CO2", "H2", "H2O", "O2", "N2"]
    for species in keys(mixture_comp)
        if mixture_comp[species] == 0.0
            species_properties[species]["specific_heat"] = calculate_specific_heat(temp, species)[1]  
            species_properties[species]["enthalpy"] = calculate_enthalpy(temp, species)[1]
            species_properties[species]["entropy"] = calculate_entropy(temp, species)[1]
            @printf("%s \t\t %+.4e \t\t %+.4e \t\t %.4e \n", species, species_properties[species]["enthalpy"], species_properties[species]["entropy"], species_properties[species]["specific_heat"])
        else
            @printf("%s \t\t %+.4e \t\t %+.4e \t\t %.4e \n", species, species_properties[species]["enthalpy"], species_properties[species]["entropy"], species_properties[species]["specific_heat"])
        end
